...
class WorldRule(ConsistencyRule):
        ...
        for rule in forbidden_statements:
            for candidate in self._extract_forbidden_candidates(rule):
                ...
                if candidate in normalized_draft:
                    severity = Severity.P0
                    reason_prefix = "违反世界规则"
                    if self._is_negated_match(normalized_draft, candidate):
                        severity = Severity.P1
                        reason_prefix = "可能涉及世界规则（否定语境）"
                    conflicts.append(
                        Conflict(
                            id=str(uuid4()),
                            severity=severity,
    ...
    def _extract_forbidden_candidates(self, rule: str) -> List[str]:
        ...
        if "不能" in normalized_rule:
            _left, right = normalized_rule.split("不能", 1)
            if right:
                candidates.append(right)
        if "禁止" in normalized_rule:
            _left, right = normalized_rule.split("禁止", 1)
            if right:
                candidates.append(right)
        ...
        for item in candidates:
            if item and len(item) >= 2 and item not in unique:
                unique.append(item)
        return unique
...
class ConsistencyEngine:
    ...
    def check(self, draft: str, context: Dict[str, Any]) -> Dict[str, Any]:
        all_conflicts = []
